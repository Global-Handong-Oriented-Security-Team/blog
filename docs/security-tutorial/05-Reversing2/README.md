...
출력 결과를 보면 같을 때 0 이 되고 다를 때 1 이 되는 XOR 연산을 확인할 수 있다. 참고로 숫자 앞에 `0b` 가 있으면 이진법, `0x` 가 숫자 앞에 있으면 16 진수, `0o` 가 앞에 있으면 8진수이다.
...
```python exec="on" source="material-block" result="ansi"
