...
튜링기계를 만드는 프로그래머는 종종 한 가지 실수를 합니다. 그 실수란 이렇습니다. "사용자가 내가 예상하는 입력만을 그 프로그램에 입력할 것이다." 그리고 예상되는 입력에 대해서만 프로그램이 잘 작동할 수 있게끔 튜링기계를 설계(코딩)합니다. 그런데 말입니다. **[ProgrammerBase](https://ccss17.netlify.app/ProgrammerBase)** 의 튜링의 증명에서 살펴보았듯 튜링기계에 입력될 수 있는 입력은 자연수의 개수만큼 무한히 많습니다.
